...
export const Shape = Item.extend(
  /** @lends Shape# */ {
    ...
    toPath: function (insert) {
      ...
      var path = new ref.Path[Base.capitalize(this._type)]({
        center: new Point(),
        size: this._size,
        radius: this._radius,
        insert: false,
      ...
      if (ref.paper.settings.applyMatrix) path.setApplyMatrix(true);
    ...
    _getBounds: function (matrix, options) {
      ...
      return strokeWidth
        ? rect.expand(ref.Path._getStrokePadding(strokeWidth, this._getStrokeMatrix(matrix, options)))
        : rect;
    },
  },
  // @ts-expect-error = 'new' expression, whose target lacks a construct signature
  new (function () {
    // Scope for _contains() and _hitTestSelf() code.
    ...
    return /** @lends Shape# */ {
      _contains: function _contains(point) {
        if (this._type === 'rectangle') {
          ...
          return center
            ...
            : (_contains as any).base.call(this, point);
        } else {
      ...
      _hitTestSelf: function _hitTestSelf(point, options, viewMatrix, strokeMatrix) {
        ...
        if (hitStroke || hitFill) {
          var type = this._type,
            ...
            strokePadding = options._tolerancePadding.add(
              ref.Path._getStrokePadding(strokeRadius, !style.getStrokeScaling() && strokeMatrix)
            );
          if (type === 'rectangle') {
        ...
        return hit
          ? new ref.HitResult(hitStroke ? 'stroke' : 'fill', this)
