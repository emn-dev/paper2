...
import { Point, Path, CompoundPath, Matrix, PathItem, Rectangle, Segment, Shape, Size } from '~/index-core';
import { equals } from './_helpers';

describe('Given: PathItem Contains', () => {
  function testPoint(item, point, inside, message = '') {
    equals(
      item.contains(point),
      inside,
      message || 'The point ' + point + ' should be ' + (inside ? 'inside' : 'outside') + '.'
    );
  }

  it('Path#contains() (regular polygon: #208)', () => {
    const path = new Path.RegularPolygon([0, 0], 6, 20);
  ...
  it('Path#contains() (circle contours)', () => {
  ...
  it('Path#contains() (transformed circle contours)', () => {
    const path = new Path.Circle({
      center: [200, 200],
      radius: 50,
      fillColor: 'blue',
  ...
  it('Path#contains() (round rectangle: #227)', () => {
    const rectangle = new Rectangle({
      point: new Point(0, 0),
      size: new Size(200, 40),
    });
    const path = new Path.Rectangle(rectangle, new Size(20, 20));
    testPoint(path, new Point(100, 20), true);
  });

  it('Path#contains() (open circle)', () => {
    const path = new Path.Circle([100, 100], 100);
    path.closed = false;
    path.fillColor = '#ff0000';
    testPoint(path, new Point(40, 160), false);
  });

  it('CompoundPath#contains() (donut)', () => {
    const path = new CompoundPath([new Path.Circle([0, 0], 50), new Path.Circle([0, 0], 25)]);
    ...
    function testDonut(path, title) {
      ...
      testPoint(
        ...
        new Point(0, -50),
        true,
        title + 'The top center point of the outer circle should be inside the donut.'
      );
      testPoint(path, new Point(0, 0), false, title + 'The center point should be outside the donut.');
      testPoint(
        path,
        new Point(-35, 0),
        true,
        title + 'A vertically centered point on the left side should be inside the donut.'
      );
      testPoint(
        path,
        new Point(35, 0),
        true,
        title + 'A vertically centered point on the right side should be inside the donut.'
      );
      testPoint(
        path,
        new Point(0, 49),
        true,
        title + 'The near bottom center point of the outer circle should be inside the donut.'
      );
      testPoint(
        path,
        new Point(0, 50),
        true,
        title + 'The bottom center point of the outer circle should be inside the donut.'
      );
      testPoint(
        path,
        new Point(0, 51),
        false,
        title + 'The near bottom center point of the outer circle should be outside the donut.'
      );
      testPoint(
        path,
        new Point({ length: 50, angle: 30 }),
        true,
        title + 'A random point on the periphery of the outer circle should be inside the donut.'
      );
      testPoint(
        path,
        new Point(-25, 0),
        true,
        title + 'The left center point of the inner circle should be inside the donut.'
      );
      testPoint(
        path,
        new Point(0, -25),
        true,
        title + 'The top center point of the inner circle should be inside the donut.'
      );
      testPoint(
        path,
        new Point(25, 0),
        true,
        title + 'The right center point of the inner circle should be inside the donut.'
      );
      testPoint(
        path,
        new Point(0, 25),
        true,
        title + 'The bottom center point of the inner circle should be inside the donut.'
      );
      testPoint(
        path,
        new Point(-50, -50),
        false,
        title + 'The top left point of bounding box should be outside the donut.'
      );
      testPoint(
        path,
        new Point(50, -50),
        false,
        title + 'The top right point of the bounding box should be outside the donut.'
      );
      testPoint(
        path,
        new Point(-50, 50),
        false,
        title + 'The bottom left point of bounding box should be outside the donut.'
      );
      testPoint(
        path,
        new Point(50, 50),
        false,
        title + 'The bottom right point of the bounding box should be outside the donut.'
      );
      testPoint(
        path,
        new Point(-45, 45),
        false,
        title + 'The near bottom left point of bounding box should be outside the donut.'
      );
    }
  ...
  it('Shape#contains()', () => {
    let shape = new Shape.Circle([0, 0], 100);
  ...
  it('Path#contains() (rectangle contours)', () => {
    const path = new Path.Rectangle(new Point(100, 100), [200, 200]),
  ...
  it('Path#contains() (rotated rectangle contours)', () => {
    const path = new Path.Rectangle(new Point(100, 100), [200, 200]),
      curves = path.getCurves();
  ...
  it('Path#contains() (touching stationary point with changing orientation)', () => {
    const path = new Path({
      segments: [
        new Segment([100, 100]),
  ...
  it('Path#contains() (complex shape: #400)', () => {
    const path = new Path({
      pathData:
  ...
  it('Path#contains() (straight curves with zero-winding: #943)', () => {
    const pointData = [
  ...
  it('CompoundPath#contains() (nested touching circles and other edge cases: #944)', () => {
    let cp = new CompoundPath({
      children: [
        new Path.Circle({
          center: [200, 200],
          radius: 100,
  ...
  it('Path#contains() with Path#interiorPoint: #854, #1064', () => {
    const paths = [
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      testPoint(
        ...
        path.interiorPoint,
        true,
        'The path[' + i + "]'s interior point should actually be inside the path"
      );
    }
  });

  it('IPathtem#contains() with non-invertible matrices (#1651)', () => {
    const path = new Path({
      matrix: new Matrix(0, 0, 0, 0, 0, 0),
    });
    equals(path.contains(path.position), false, 'A path with a non-invertible matrix cannot contain its position');
  });
});
