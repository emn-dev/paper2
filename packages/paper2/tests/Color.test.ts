...
describe('Given: Color Class', () => {
  describe('WHEN set named color', () => {
    it('THEN should be success', () => {
      const path = new p2.Path();
      path.fillColor = 'red';

      // equals(path.fillColor, new Color(1, 0, 0));
      expect(path.fillColor.type).toBe(new p2.Color(1, 0, 0).type);
      expect(path.fillColor.components).toEqual(new p2.Color(1, 0, 0).components);

      // equals(path.fillColor.toCSS(), "rgb(255,0,0)");
      expect(path.fillColor.toCSS()).toBe('rgb(255,0,0)');
    });
  });

  describe('WHEN set color to hex', () => {
    it('THEN should be success', () => {
      const path = new p2.Path();
      ...
      // equals(path.fillColor, new p2.Color(1, 0, 0));
      expect(path2.fillColor.type).toBe(new p2.Color(1, 0, 0).type);
      expect(path2.fillColor.components).toEqual(new p2.Color(1, 0, 0).components);

      // equals(path.fillColor.toCSS(), 'rgb(255,0,0)');
      expect(path2.fillColor.toCSS()).toBe('rgb(255,0,0)');
    });
  });
});
