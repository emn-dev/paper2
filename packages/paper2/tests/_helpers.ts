...
function compareImageData(imageData1, imageData2, tolerance, message = '', description = '', cb = {} as any) {
  ...
  // if (numDiffPixels > 0) {
  // Give a little tolerance, if only 24 pixels or less do not match then we still pass the test
  if (numDiffPixels > 25) {
...
// //   return (obj instanceof Base ? QUnit.jsDump.parsers.Base : objectParser).call(this, obj, stack);
// // });
...
//   //   var id = QUnit.config.current.testId;
//   //     var index = QUnit.config.current.assertions.length + 1;
//   var result;
//   //   // Compare image-data using resemble.js:
//   //   resemble.compare(
//   //     imageData1,
...
// //     // Get the right entry for this unit test and assertion, and
// //     // replace the results with images
// //     var entry = document.getElementById('qunit-test-output-' + id).querySelector('li:nth-child(' + index + ')'),
// //       bounds = result.diffBounds;
// //     entry.querySelector('.test-expected td').appendChild(image(imageData2));
// //     entry.querySelector('.test-actual td').appendChild(image(imageData1));
// //     entry.querySelector('.test-diff td').innerHTML =
// //       '<pre>' + text + (description || '') + '</pre><br>' + '<img src="' + result.getImageDataUrl() + '">';
// //   }
// };
...
//     //   null,
//     //   null,
//     //   'Unable to compare rasterized items: ' + (!actualRaster ? 'actual' : 'expected') + ' item is null',
//     //   QUnit.stack(2)
//     // );
//   } else {
//     // Compare the two rasterized items.
...
//     var sharedProject = !(options && options.dontShareProject);
//     QUnit.push(
//       sharedProject ? sameProject : !sameProject,
//       actual.project,
//       sharedProject ? expected.project : 'not ' + expected.project,
//       message + ' (#project)'
//     );
...
//       // Cloning SymbolItems does not result in cloned
//       // SymbolDefinitions
//       options && options.cloned ? Base.set({}, options, { cloned: false }) : options,
//       ['symbol']
//     );
//   },
...
//   var MouseEvent = typeof nativeClasses.MouseEvent === 'function' ? nativeClasses.MouseEvent : MouseEventPolyfill;
