...
describe('Given: Curve Class', () => {
  ...
  it('Curve#getNormalAtTime()', () => {
    ...
    for (let i = 0; i < normals.length; i++) {
      ...
      equals(curve.getNormalAtTime(entry[0]), entry[1].normalize(), 'curve.getNormalAtTime(' + entry[0] + ');');
      equals(curve.getWeightedNormalAtTime(entry[0]), entry[1], 'curve.getWeightedNormalAtTime(' + entry[0] + ');');
      // Legacy version:
      equals(
  ...
  it('Curve#getCurvatureAtTime()', () => {
    ...
    for (let i = 0; i < curvatures.length; i++) {
      ...
      equals(curve.getCurvatureAtTime(entry[0]), entry[1], 'curve.getCurvatureAtTime(' + entry[0] + ');');
      // Legacy version:
      equals(curve.getCurvatureAt(entry[0], true), entry[1], 'Legacy: curve.getCurvatureAt(' + entry[0] + ', true);');
    }
  });

  it('Curve#getCurvatureAtTime() b', () => {
  ...
  it('Curve#getTimeAt()', () => {
    ...
    for (let f = 0; f <= 1; f += 0.1) {
      ...
      const message = 'Curve-time parameter at offset ' + o1 + ' should be the same value as at offset ' + o2;
      equals(t1, t2, message, Numerical.CURVETIME_EPSILON);
      equals(function () {
        return curve.getOffsetAtTime(t1);
      }, o1);
      equals(function () {
        return curve.getOffsetAtTime(t2);
      }, curve.length + o2);
      // Legacy version:
      equals(curve.getParameterAt(o1), curve.getParameterAt(o2), 'Legacy: ' + message, Numerical.CURVETIME_EPSILON);
      // Test other methods with negatives offsets
      equals(
        curve.getTangentAt(o1),
        curve.getTangentAt(o2),
        'Tangent at offset ' + o1 + ' should be the same value as at offset ' + o2
      );
    }

    equals(curve.getTimeAt(curve.length + 1), null, 'Should return null when offset is out of range.');
  });

  it('Curve#getTimeAt() with straight curve', () => {
    const path = new Path();
    path.moveTo(100, 100);
    path.lineTo(500, 500);
  ...
  it('Curve#getTimeAt() with straight curve b', () => {
    // #1000:
    const curve = new Curve([
      1584.4999999999998, 1053.2499999999995, 1584.4999999999998, 1053.2499999999995, 1520.5, 1053.2499999999995,
      1520.5, 1053.2499999999995,
    ]);
    const offset = 63.999999999999716;
    equals(function () {
      return offset < curve.length;
    }, true);
    equals(function () {
      return curve.getTimeAt(offset);
    }, 1);
  });

  it('Curve#getTimeAt() with offset at end of curve', () => {
    // #1149:
    const curve = [-7500, 0, -7500, 4142.135623730952, -4142.135623730952, 7500, 0, 7500];
    equals(Curve.getTimeAt(curve, 11782.625235553916), 1);
  });

  it('Curve#getLocationAt()', () => {
    const curve = new Path([
      [
        [0, 0],
        [0, 0],
    ...
    equals(curve.getLocationAt(curve.length + 1), null, 'Should return null when offset is out of range.');
  });
    ...
    for (let pos = 0; pos < path.length; pos += 10) {
      ...
      for (let i = 0; i < path.curves.length; i++) {
        ...
        if (time) {
          // Legacy-check-hack:
          equals(
            curve.getParameterOf(point1),
            time,
            'Legacy: curve.getParameterOf() should return the same' + ' as curve.getTimeOf()'
          );
          point2 = curve.getLocationAtTime(time).point;
          break;
        }
      }
      equals(point1, point2, 'curve.getLocationAt(curve.getTimeOf(' + point1 + ')).point;');
    }
  });

  it('Curve#getPartLength() with straight curve', () => {
    const curve = new Curve([0, 0, 0, 0, 64, 0, 64, 0]);
    equals(function () {
      return curve.getPartLength(0.0, 0.25);
    }, 10);
    equals(function () {
      return curve.getPartLength(0.25, 0.5);
    }, 22);
    equals(function () {
      return curve.getPartLength(0.25, 0.75);
    }, 44);
    equals(function () {
      return curve.getPartLength(0.5, 0.75);
    }, 22);
    equals(function () {
      return curve.getPartLength(0.75, 1);
    }, 10);
  });

  it('Curve#divideAt(offset)', () => {
  ...
  it('Curve#getTimesWithTangent()', () => {
    ...
    equals(
      curve.getTimesWithTangent([-1, 0]),
      [0],
      'should return the same when called with opposite direction vector'
    );
